...
function SelectSearchField({name,dataFields,defaultValue,styleClass}) {
    ...
    return (
        <div className='relative w-full'>
            <input hidden key={selected} onInput={OnInput} name={name} id={name} placeholder={name} defaultValue={selected} autoFocus type="text" className='h-11 px-3 w-full text-lg text-white appearance-none bg-transparent outline-none border-b border-b-[#334053] focus:border-b-[#73fdea]'/>
            <div onClick={()=>{setIsActive(!isActive)}} className={styleClass}>
                {
                    isActive?
                    <input key={selected} onInput={OnInput} name={name} id={name} placeholder={name} defaultValue={selected} autoFocus type="text" className='h-11 px-3 w-full text-lg text-white appearance-none bg-transparent outline-none border-b border-b-[#334053] focus:border-b-[#73fdea]'/>
            ...
            </div>
            {
                ...
                <div className='max-h-[280px] overflow-scroll overflow-x-hidden z-50 w-full absolute top-11 rounded-sm  border-gray-100 bg-white'>
                    ...
                    return (  
                        <div onClick={()=>{OnSelect(item)}}  key={key} className="w-full rounded-t cursor-pointer hover:bg-gray-100">  
